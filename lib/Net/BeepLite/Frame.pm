# $Id: Frame.pm,v 1.5 2003/09/05 18:37:18 davidb Exp $
#
...
=head1 SYNOPSIS
  ...
  my $frame1 = Net::BeepLite::Frame->new
   (Header => $header,
    Payload => $payload);

  my $frame2 = Net::BeepLite::Frame->new
   (Buffer => $header_plus_payload);

  my $frame3 = Net::BeepLite::Frame->new
   (Type => "MSG",
...
"Net::BeepLite::Frame" is a class used for describing BEEP frames, the
underlying unit of transport in BEEP.  This is generally not used in
user code.  Instead, it is used internally by the
C<Net::BeepLite::Session> class for sending and receiving messages.
