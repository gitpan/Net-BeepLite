...
BEGIN { use_ok('Net::BeepLite::MgmtProfile');
...
# Testing Net::BeepLite::MgmtProfile

# the constructor:
my $mgmt = Net::BeepLite::MgmtProfile->new;
ok(defined $mgmt, 'constructor works');
isa_ok($mgmt, 'Net::BeepLite::MgmtProfile');
isa_ok($mgmt, 'Net::BeepLite::BaseProfile');

$mgmt = Net::BeepLite::MgmtProfile->new(AllowMultipleChannels => 1);
...
bless $fake_profile, 'Net::BeepLite::BaseProfile';

# a session.
my $session = new Net::BeepLite::Session();
...
isa_ok($msg, 'Net::BeepLite::Message', 'mgmt->greeting_message()');
...
SKIP:
{
  ...
  sub msg_from_lines {
    my $header = shift;
    my $frame = new Net::BeepLite::Frame(Header => $header);
    my $payload = join('', @_);
    $frame->set_payload(substr($payload, 0, $frame->size()));
    new Net::BeepLite::Message(Frame => $frame);
  }
  ...
  bless $fake_profile2, 'Net::BeepLite::BaseProfile';
  my $fake_profile3 = { uri => "http://host.example.com/some/profile3" };
  bless $fake_profile3, 'Net::BeepLite::BaseProfile';
