...
isa_ok($channel_2, 'Net::BeepLite::Channel');
...
SKIP:
{
  ...
  $session = new Net::BeepLite::Session(Socket => $fake_socket);
  ...
  my $frame_expect_result = "ANS 2 2 . 120 $frame_payload_size 0\r\n" .
    $frame_payload . "END\r\n";
  my $frame = new Net::BeepLite::Frame
    (Type    => 'ANS',
     Channel => 2,
     Msgno   => 2,
     Ansno   => 0,
  ...
  $frame = new Net::BeepLite::Frame(Type => 'MSG',
				    Channel => 2,
				    Msgno => 3,
				    More => '.',
				    Seqno => 200,
  ...
  my $ans_frame = new Net::BeepLite::Frame
    (Type    => 'ANS',
     Channel => 2,
     Msgno   => 2,
     Ansno   => 12,
  ...
  my @msgs;
  for (my $i = 0; $i < 3; $i++) {
    $msgs[$i] = new Net::BeepLite::Message(Type    => 'MSG',
					   Msgno   => 1,
					   Channel => ($i * 2) + 1,
  ...
  for (my $i = 0; $i < 4; $i++) {
    @msgs[$i] = new Net::BeepLite::Message(Type    => 'ANS',
					   Msgno   => 12,
					   Ansno   => $i == 3 ? 0 : $i,
					   Channel => $i == 3 ? 3 : 1,
					   Content => $full_content);
  ...
  my $outgoing_msg = new Net::BeepLite::Message(Type 	=> 'MSG',
						Channel => 5,
						Content => $full_content);
  my $incoming_msg = new Net::BeepLite::Message(Type 	=> 'RPY',
						Msgno 	=> 1,
						Channel => 5,
						Content => $full_content);
  # construct input.
